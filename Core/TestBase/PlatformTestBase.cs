...
namespace Automation.UI.Core.TestBase
{
    ...
    /// </summary>
    public class PlatformTestBase : WebUITestBaseClass
    {
        ...
        public void TryToLogOut()
        {
            try
            {
                if (Driver != null)
                {
                    ...
                    if (!Driver.Url.Contains(PlatformBaseURL))
                    {
                        Driver.Navigate().GoToUrl(PlatformBaseURL);
                    }

                (new HeaderSubPage(Driver, PlatformBaseURL)).LogOut();
                }
            }
